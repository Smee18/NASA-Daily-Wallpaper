...
for i, photo in enumerate(mars_photos, start=1):
    ...
    if img_data_mars.status_code == 200:
        ...
        raise Exception(f"Failed to download image: {img_data_mars.status_code}")

# Extract APOD
img_data_APOD = requests.get(image_url)
if img_data_APOD.status_code == 200:
    APOD_image_path = f'images/{image_date}.jpg'
    with open(APOD_image_path, 'wb') as file:
        file.write(img_data_APOD.content)
    print(f"Saved: {APOD_image_path}")
else:
    raise Exception(f"Failed to download image: {img_data_APOD.status_code}")
...
for img in mars_files: # Obtain at most 4 pics from NAV cams. Not best code as doesn't format nicely if not enough pics
...
# --- RIGHT HALF: ROVER PICS (2x2 grid) ---
cols, rows = 2, 2
